  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="rounded-xl bg-[var(--color-error-light)] border border-[var(--color-error-heart)]/30 px-4 py-3">
        <p className="font-fredoka text-sm text-[var(--color-text-secondary)] leading-relaxed whitespace-pre-line">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {relatedTopics.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 rounded-full bg-[var(--color-error-light)] text-[var(--color-error-heart)] font-fredoka text-sm"
              >
                {tag}
              </span>
            ))}
